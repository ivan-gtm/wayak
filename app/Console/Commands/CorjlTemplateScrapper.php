...
class CorjlTemplateScrapper extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'wayak:corjl:scrapper';
    ...
    function generateWayakTemplate() {
        ...
        foreach ($product as $product_key) {
            $template_svg_path = public_path('/design/template/'.str_replace('corjl:',null, $product_key).'/svg_template_0_page_1.svg');
                ...
                if( isset( $db_template->template_id ) == false 
                    ...
                    if( isset($templates_obj->templates) ){
                        ...
                        for ($template_index=0; $template_index < $total_products; $template_index++) {
                            echo "\n\t>> $original_product_key parsing page $template_index";
                            ...
                            if( isset( $templates_obj->templates[$template_index]->pages )
                                ...
                                for ($page_index=1; $page_index <= $total_pages; $page_index++) {
                                    ...
                                    $svg_file_path = public_path('/design/template/'.$original_product_key.'/'.$svg_filename);
                                    if( file_exists($svg_file_path) && filesize($svg_file_path) > 0) {
                                        
                                        echo "\n\t>> Parsing $svg_file_path";
                                        ...
                                        Redis::del('corjl:'.$original_product_key);
                                    }
                                    
                                }

                                if( $incomplete_template == false ){
                                    ...
                                    print_r("\n<< FINISH >>".'template:en:'.$new_template_id.':jsondata'."\n\n\n");
                                }
                    ...
                    echo "\n\n\t>> $original_product_key does NOT exists on thumbnails table ";
                    exit;
                }
            } else {
